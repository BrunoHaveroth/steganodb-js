...
var Decoder = require('../cript/decoder.js');
...
module.exports = {
  read: function (path, key) {
    ...
    .then(function (image) {
      ...
      text = that.getDecodeText(text, key);
      return text;
    });
  },
  ...
  _getValueOfRgb: function(rgbProperty) {
    if (rgbProperty === undefined) throw { message: 'A chave do banco de dados está incorreta' };
  ...
  getDecodeText: function(text, key) {
    var decodeType = (text.length %2 === 0) ? 'substitution' : 'transposition';
    return Decoder.decode(text, key, decodeType);
  }
};
