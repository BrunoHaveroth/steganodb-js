...
var Encoder = require('../cript/encoder.js');

module.exports = {
  write: function(path, key, text) {
    var that = this;
    text = this.getEncodeText(text, key);
  ...
  getEncodeText: function(text, key) {
    var encodeType = (text.length %2 === 0) ? 'substitution' : 'transposition';
    return Encoder.encode(text, key, encodeType);
  }
};
